...
class FieldValueSerializer(serializers.ModelSerializer):
    """Serializes the value of one field in an argument."""
...
class ArgumentSummarySerializer(serializers.ModelSerializer):
    """Serializes basic info about an argument for lists and links."""
    ...
    class Meta:
        model = Argument
        fields = ('id', 'author', 'theme', 'scheme_name', 'scheme_template', 'field_values', 'date_created', 'child_count', 'is_winning')

    def get_child_count(self, obj):
        return obj.child_links.count()

class ChildArgumentSerializer(serializers.ModelSerializer):
    """Serializes an argument linked to a parent, including the link info."""
...
class ArgumentDetailSerializer(serializers.ModelSerializer):
    """Serializes all details of an argument, including linked attackers and supporters."""
    author = serializers.CharField(source='author.username', read_only=True)
    theme = serializers.CharField(source='theme.title', read_only=True)
    ...
    class Meta:
        model = Argument
        fields = ('id', 'author', 'theme', 'theme_id', 'scheme_name', 'scheme_id', 'scheme_template', 'field_values', 'date_created', 'attackers', 'supporters', 'reported', 'report_count', 'is_winning')

    def get_attackers(self, obj):
        links = obj.child_links.filter(attacking=True).select_related('child_argument', 'critical_question', 'child_argument__author', 'child_argument__theme', 'child_argument__scheme')
        return ChildArgumentSerializer(links, many=True).data

    def get_supporters(self, obj):
        links = obj.child_links.filter(attacking=False).select_related('child_argument', 'critical_question', 'child_argument__author', 'child_argument__theme', 'child_argument__scheme')
        return ChildArgumentSerializer(links, many=True).data
...
class ArgumentFieldValueInputSerializer(serializers.Serializer):
    """Validates the input for one field value when creating an argument."""
    scheme_field_id = serializers.IntegerField()
    value = serializers.CharField()

class CreateArgumentSerializer(serializers.Serializer):
    """Validates input for creating a new argument, including field values and optional parent link."""
    ...
    def validate(self, data):
        ...
        if has_parent and not has_question:
            raise serializers.ValidationError({'critical_question_id': 'A critical question is required for non-initial arguments.'})
        if has_question and not has_parent:
            raise serializers.ValidationError({'parent_argument_id': 'A parent argument is required when a critical question is provided.'})
        scheme_id = data['scheme_id']
        field_ids = [fv['scheme_field_id'] for fv in data['field_values']]
        valid_ids = set(SchemeField.objects.filter(scheme_id=scheme_id).values_list('id', flat=True))
        invalid = set(field_ids) - valid_ids
        if invalid:
            raise serializers.ValidationError({'field_values': f'Field IDs {invalid} do not belong to the selected scheme.'})
        return data
