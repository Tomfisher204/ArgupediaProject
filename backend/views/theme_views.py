...
class ThemeListView(APIView):
    """Returns a paginated list of argument themes, with optional search and sorting."""
    ...
    def get(self, request):
        ...
        if search:
            themes = themes.filter(Q(title__icontains=search) | Q(description__icontains=search))
        themes = themes.annotate(initial_argument_count=Count('argument', filter=Q(argument__parent_links__isnull=True)))
...
class ThemeArgumentsView(APIView):
    """Returns the initial arguments for a specific theme."""
    permission_classes = [IsAuthenticated]
