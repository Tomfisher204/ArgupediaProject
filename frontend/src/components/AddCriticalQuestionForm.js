  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/admin/critical-questions/`, {
        ...
        body: JSON.stringify({ ...form, scheme_id: schemeId }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data?.question?.[0] ?? 'Failed to create critical question.');
      }
      onSuccess();
    } catch (err) {
    ...
    <>
      <div className="modal-body">
        <div className="form-field">
          <label className="form-label" htmlFor="cq-question">Critical question</label>
          <textarea
            id="cq-question"
            name="question"
